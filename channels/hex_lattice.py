...
class HexagonalLatticeChannel(Channel):
    """
        A channel produced by a 2D hexagonal (brickwork) lattice cluster state
    """
    ...
    def __init__(self, width, length, prob_edge=1, prob_loss=0,
        ...
        r_edges = [(i, i + 1) for i in nodes[:-1] if i % 2]
        self.edges += r_edges
    ...
    def add_layer(self, layer):
        ...
        new_edges = [(i, i + 1) for i in new_nodes[:-1] if (i + layer - 1) % 2]
...
if __name__ == '__main__':
    # ============ SINGLE-SHOT TESTING OF HexagonalLatticeChannel ============
    width, length = 3, 3
    psi = HexagonalLatticeChannel(width, length)
